...
jump = ['jmp']
def symbol_table(list1):
    symbol = []
    label = []
    data_addr = 0
    bss_addr = 0
    for i in range(len(list1)):
        #for data and bss section 
        if (len(list1[i])>2):
            if (list1[i][1] in data_type + ['db']):
                number = length(list1[i][1],list1[i][2])
                size = number * size_dict[list1[i][1]] 
                symbol.append([i+1, list1[i][0], size_dict[list1[i][1]], number,'S','D', hex(data_addr),list1[i][2]])
                data_addr = data_addr + size
            if (list1[i][1] in bss_type):
                number = int(list1[i][2])
                size = number * size_dict[list1[i][1]] 
                symbol.append([i+1, list1[i][0], size_dict[list1[i][1]], number,'S','R', hex(bss_addr),'-'])
        ...
        if (len(list1[i]) > 1):
            if (list1[i][0] == 'global') or (list1[i][0] in jump):
                if (list1[i][1] in defined_labels) and (list1[i][1] not in temp_label):
                    label.append([i+1, 'label#'+str(count_label), list1[i][1], 'D'])
                    count_label += 1
                    temp_label.append(list1[i][1])
                else:
                    label.append([i+1, 'label#'+str(count_label), list1[i][1], 'U'])
                    count_label += 1
        if ((list1[i][0][:len(list1[i][0])-1]) in defined_labels) and ((list1[i][0][:len(list1[i][0])-1]) not in temp_label):
            label.append([i+1, 'label#'+str(count_label), list1[i][0][:len(list1[i][0])-1], 'D'])
            count_label += 1
